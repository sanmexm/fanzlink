...
const ResetPassword = () => {
    ...
    //use useEffect to check if the id, email and passwordResetToken corresponds

    const handleSubmit = async (e) => {
      e.preventDefault();
      const returnConfirm = window.confirm('Are you sure about the update?');
      
      if (returnConfirm) {
        try {
          setSavingInfo(true);
          const response = await dispatch(updatePassword(id, postData));
